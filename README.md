...
Minecraft Java 版 **1.21.11 Fabric** 客户端模组：当游戏语言为 **简体中文（zh_cn）、繁體中文-台灣（zh_tw）、繁體中文-香港（zh_hk）、文言文（lzh）** 之一时，在**所有单人存档**解锁飞行权限——不管是生存、冒险还是其他模式，都可以像创造模式一样**双击空格**起飞。

把语言切换成非中文后会**立即自动收回**飞行权限（创造/旁观模式自带的飞行不受影响）。改语言不用重启游戏，切完即生效。

- 仅客户端模组，只在单人存档（集成服务端）生效；进入多人服务器时不做任何事。
- 你自己"对局域网开放"的房间也算单人存档：房主装了本模组后，房间内所有玩家都会获得飞行权限。
